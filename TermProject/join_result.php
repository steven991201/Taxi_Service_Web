...
$sql = "select RoomId from Taxi_Room";
...
if ($res->num_rows>0){
    ...
    if (!(in_array($room,$list))){
        ...
        echo "<a href='join.php'><input type='button' value='Room list'></a>";
    }
    else{
        ...
        $sql = "insert into Taxi_Guest (TimeStamp, User_UserId) values ('{$now}','{$user}')";
        $res = mysqli_query($conn, $sql);
        $id = mysqli_insert_id($conn);

        $sql = "insert into Taxi_Join (Taxi_Guest_guestId, Taxi_Room_RoomId) values ('{$id}', '{$room}')";
        mysqli_query($conn, $sql);

        $sql = "select GN from Taxi_Room where RoomId='$room'";
        $res = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($res);
        $current = $row['GN'];

        $new = $current + 1;
        $sql = "UPDATE Taxi_Room SET GN='$new' where RoomId='$room'";
        mysqli_query($conn, $sql);

        if($new==4){
            $sql = "UPDATE Taxi_Room SET Status='Closed' where RoomId='$room'";
            mysqli_query($conn, $sql);
        }
